...
input_cereals <- read.csv("cereal.csv", header = TRUE, sep = ",", dec = ".")
...
input_cereals$totalcarb <- input_cereals$Carbs + input_cereals$Sugars

# 4. Crea un nuevo data.frame "kelloggs" que recoja solo las filas del dataset 
# cuyo manufacturador sea "Kelloggs".

kellogs <- input_cereals[input_cereals$Manufacturer == "Kelloggs",]
...
no_health <- input_cereals[input_cereals$Calories > 80 & input_cereals$Vitamins.and.Minerals < 20,]
...
cereal_sugar <- input_cereals[input_cereals$Sugars >= 1, c("Cereal.Name" ,"Calories", "Vitamins.and.Minerals")]
